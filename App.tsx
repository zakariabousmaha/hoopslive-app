...
import { Menu, Search, Loader2, CalendarDays, ChevronLeft, ChevronRight, X, Trophy, Calendar, Star, CircleDot, Zap, Tv, Sparkles, Activity, PieChart, History, Shield } from 'lucide-react';
...
// ==========================================
// 1. SHARED COMPONENTS (Defined Internally)
// ==========================================
...
const AdUnit: React.FC<{ slotId: string; format?: string; className?: string; style?: any; label?: string }> = ({ slotId, format = 'auto', className = '', style = {}, label = 'Advertisement' }) => {
  useEffect(() => { try { (window as any).adsbygoogle = (window as any).adsbygoogle || []; (window as any).adsbygoogle.push({}); } catch (e) { /* Ignore */ } }, []);
  if (slotId === 'DEMO_SLOT') {
    return (
...
// ==========================================
// 2. COMPLEX UI COMPONENTS (Internalized)
// ==========================================
...
const StatRow = ({ label, homeVal, awayVal, suffix = '', invert = false }: any) => {
  ...
  return (
    <div className="bg-slate-850 p-3 rounded border border-slate-800">
      <div className="flex justify-between text-xs font-bold mb-1"><span className={homeIsBetter ? 'text-white' : 'text-slate-400'}>{homeVal}{suffix}</span><span className="text-slate-500 font-normal uppercase">{label}</span><span className={!homeIsBetter ? 'text-white' : 'text-slate-400'}>{awayVal}{suffix}</span></div>
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-slate-900 border-r border-slate-800 z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:block`}>
        <div className="flex items-center gap-3 px-4 h-16 border-b border-slate-800"><div className="w-8 h-8 bg-gradient-to-tr from-hoops-orange to-red-500 rounded flex items-center justify-center text-white font-black italic">H</div><span className="text-xl font-bold tracking-tight text-white">Hoops<span className="text-hoops-orange">Live</span></span></div>
        <div className="p-4 overflow-y-auto h-[calc(100vh-64px)]">
          ...
          <div className="mt-auto px-2"><AdUnit slotId="DEMO_SLOT" format="rectangle" style={{ height: '250px', width: '100%' }} className="w-full h-[250px] bg-slate-850 rounded" /><p className="text-[10px] text-center text-slate-600 mt-2">Sponsored Partner</p></div>
        </div>
      </div>
    </>
  );
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-900 relative z-10 overflow-x-hidden">
        ...
        {!selectedTeamProfile && !searchQuery && (
            <div className="flex items-center justify-between px-2 sm:px-4 py-2 bg-slate-850 border-b border-slate-800 text-xs font-medium text-slate-400 relative z-30">
                ...
                <div className="relative"><button onClick={() => setShowCalendar(!showCalendar)} className="text-white bg-slate-800 px-4 py-1.5 rounded flex items-center gap-2 border border-slate-700 min-w-[160px] justify-center"><CalendarDays size={14} className="text-hoops-orange" /><span>{currentDate.toLocaleDateString()}</span></button>{showCalendar && <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 z-50"><CalendarWidget selectedDate={currentDate} onSelectDate={(d) => { setCurrentDate(d); setShowCalendar(false); }} onClose={() => setShowCalendar(false)} /></div>}</div>
        ...
        <div className="flex-1 overflow-y-auto pb-20 md:pb-0">
           {selectedTeamProfile ? (
               ...
               Object.keys(groupedMatches).map(lid => <div key={lid} className="mb-2"><LeagueHeader name={groupedMatches[lid][0].leagueName} country={groupedMatches[lid][0].country} flag="🏀" /><div className="flex flex-col">{groupedMatches[lid].map(m => <MatchRow key={m.id} match={m} onClick={setSelectedMatch} isFavorite={favorites.has(m.id)} onToggleFavorite={toggleFavorite} />)}</div></div>)
           )}
        </div>
      </main>
